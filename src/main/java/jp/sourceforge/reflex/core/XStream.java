...
public class XStream {
	...
	private ClassLoaderReference classLoaderReference; // TODO: Should be
														// changeable
	...
	/**
	 * @deprecated As of XStream 1.1.1, a default Converter is unnecessary as
	 *             you can register a Converter with an associated priority. Use
...
//		setMode(XPATH_REFERENCES);
	...
	public XStream(ReflectionProvider reflectionProvider,
			ClassMapper classMapper, HierarchicalStreamDriver driver,
			String classAttributeIdentifier, Converter defaultConverter) {
	...
	private ClassMapper buildMapper(String classAttributeIdentifier) {
		MapperWrapper mapper = new DefaultMapper(classLoaderReference,
				classAttributeIdentifier);
		mapper = new XmlFriendlyMapper(mapper);
		mapper = new ClassAliasingMapper(mapper);
		classAliasingMapper = (ClassAliasingMapper) mapper; // need a reference
															// to that one
		mapper = new FieldAliasingMapper(mapper);
		fieldAliasingMapper = (FieldAliasingMapper) mapper; // need a reference
															// to that one
		mapper = new ImplicitCollectionMapper(mapper);
		implicitCollectionMapper = (ImplicitCollectionMapper) mapper; // need a
																		// reference
																		// to
																		// this
																		// one
		...
		defaultImplementationsMapper = (DefaultImplementationsMapper) mapper; // and
																				// that
																				// one
	...
	protected void setupAliases() {
		...
		// undesirable. Calling loadClass ensures
	...
	protected void setupConverters() {
		ReflectionConverter reflectionConverter = new ReflectionConverter(
				classMapper, reflectionProvider);
		registerConverter(reflectionConverter, PRIORITY_VERY_LOW);

		registerConverter(new SerializableConverter(classMapper,
		...
		registerConverter(new DynamicProxyConverter(classMapper,
	...
	private void dynamicallyRegisterConverter(String className, int priority,
		...
		try {
			Class type = Class.forName(className, false, classLoaderReference
					.getReference());
			Constructor constructor = type
					.getConstructor(constructorParamTypes);
			Converter converter = (Converter) constructor
					.newInstance(constructorParamValues);
			registerConverter(converter, priority);
		} catch (Exception e) {
			throw new InitializationException(
					"Could not instatiate converter : " + className, e);
		}
	}
	 ...
	 * given root object instead of instantiating a new one.
	 */
	 ...
	 * 
	 * For example, java.util.ArrayList is the default implementation of
	 * java.util.List.
	 * 
	 * @param defaultImplementation
	 * @param ofType
	...
	/**
	 * @deprecated As of 1.1.1 you should register a converter with the
...
/**
	 ...
	 * @param fieldName
	 *            name of the field in the ownerType. This filed must be an
...
/**
	...
	public ObjectOutputStream createObjectOutputStream(Writer writer)
			throws IOException {
		return createObjectOutputStream(new PrettyPrintWriter(writer),
				"object-stream");
	}
